...
export async function GET(request: Request, context: { params: { taskId: string } }) {
  ...
  const response = await fetchMagnific(`/v1/ai/image-change-camera/${taskId}`, apiKey, { method: "GET" })
  const payload = await readMagnificJson(response)

  if (!response.ok) {
    return NextResponse.json(
      { error: "Magnific change camera status request failed.", code: "magnific_status_failed", detail: payload },
